...
const company = {
  department: {
    admin: { employee: { emp1: 'krishna', emp2: 'Vinita' } },
    member: { employee: { emp1: 'eva' } },
  },
};
...
function addEmployee(department, employee) {
  if (company.department[department]) {
    const empKey =
      'emp' + (Object.keys(company.department[department].employee).length + 1);
    company.department[department].employee[empKey] = employee;
    console.log(`Added ${employee} as ${empKey} to ${department} department`);
  } else {
